...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
	...
	if (rightChild <= last) {  /* we have two children */
		...
		int smallest;		 /* get the index of the smallest child */
		...
		if (compare(heap->data[smallest], heap->data[position]) < 0) { /* if value at pos > value of smallest child */

			dySwap(heap, position, smallest);		 /* swap with smallest child */
		...
		if (compare(heap->data[leftChild], heap->data[position]) < 0) {  /* if value at pos > value of child */

			dySwap(heap, position, leftChild);			   /* swap with smallest child */
...
void buildHeap(DynamicArray* heap, compareFunction compare)
{
	...
	int i;
	for (i = max / 2 - 1; i >= 0; i--) {
		adjustHeap(heap, max - 1, i, compare);
	}
}
...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
	...
	int idx = dySize(heap) - 1;		/* start at the end */
	int last = idx;
	...
	*/
	...
	/* starting at the end node, go up the tree calling adjustHeap until
	either the top of the tree is reached or the value of the parent node
	is less than that of the child node */
	while ((idx / 2 - 1 + idx % 2) >= 0 && compare(heap->data[idx], heap->data[idx / 2 - 1 + idx % 2]) < 0) {
		dySwap(heap, idx, idx / 2 - 1 + idx % 2);
		idx = idx / 2 - 1 + idx % 2;
		adjustHeap(heap, last, idx, compare);
	}
}
...
void dyHeapRemoveMin(DynamicArray* heap, compareFunction compare)
{
	...
	assert(last != -1); /* make sure we have at least one element */
