...
struct dynArrayIterator {
	struct DynArr * da;			// pointer to array
	int currentIndex;			// index of array
};



/**********************************************************************
						createDynArrayIterator
	param:	da		pointer to the dynamic array
	pre:	da is not pointing to null
	post:	creates an dynArrayIterator struct and returns it
**********************************************************************/
struct dynArrayIterator * createDynArrayIterator(struct DynArr * da) {
	
	/* validate pointer is not pointing to NULL */
	assert(da != 0);
	
	/* allocate memory for new iterator struct */
	struct dynArrayIterator * newItr = malloc(sizeof(struct dynArrayIterator));

	/* confirm memory was allocated */
	assert(newItr != 0);

	/* call dynArrayIteratorInit and return the iterator */
	dynArrayIteratorInit(da, newItr);
...
/**********************************************************************
						dynArrayIteratorInit
	param:	da		pointer to the dynamic array
	param:	itr		pointer to the dynamic array iterator

**********************************************************************/

void dynArrayIteratorInit(struct DynArr *da, struct dynArrayIterator *itr) {
	itr->da = da;
	itr->currentIndex = -1;		/* index starts one before first element */

}

/**********************************************************************
				        dynArrayIteratorHasNext
param:	itr		pointer to the dynamic array iterator

**********************************************************************/
int dynArrayIteratorHasNext(struct dynArrayIterator *itr) {
	
	if (itr->currentIndex <= itr->da->size - 2) {
		itr->currentIndex++;
		return 1;
	}
...
void dynArrayIteratorRemove(struct dynArrayIterator *itr) {
	...
	itr->currentIndex--;		/* need to go back one element so that
								 nothing is skipped */
...
int main() {
	...
	struct dynArrayIterator * myIter;
	myIter = createDynArrayIterator(myArray);

	while (dynArrayIteratorHasNext(myIter) != 0) {
		TYPE val = dynArrayIteratorNext(myIter);
		if (val != 0) {
			printf("The next value in the array is: %d\n", val);
		}
		else {
			printf("The next value in the array is: %d\n", val);
			dynArrayIteratorRemove(myIter);
		}
		
	}

	printf("After removing 0 \n");
	dynArrayIteratorInit(myArray, myIter);

	while (dynArrayIteratorHasNext(myIter) != 0) {
		printf("The next value in the array is: %d\n", dynArrayIteratorNext(myIter));
	}
