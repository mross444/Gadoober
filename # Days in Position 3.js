...
var pro1 = [Calculated Proration % - Position 1];
var pro2 = [Calculated Proration % - Position 2];
...
var rs1 = ly.CreateQuery('select convert(varchar, Date6, 101) as custom56 from PersonOptional2 where personfk = ' + lRsrcID);
var rs = rs1.GetCursor();

if (!rs.EOF){
   startdate = rs("custom56");
}
...
var rs = ly.CreateQuery('select convert(varchar, Date5, 101) as custom55 from PersonOptional1 where personfk = ' + lRsrcID);
var rs1 = rs.GetCursor();

if (!rs1.EOF){
   startdate1 = rs1("custom55");
}
